...
router.post('/order/insert', async (ctx, next) => {
    ...
    await mongodb.insert('bill', { userId: inparam.userId, type: 'OUT', amount:0, createdAt: Date.now() ,remark:inparam.id,project:'购买产品'})
    await mongodb.insert('serverBill', { userId: inparam.userId, type: 'IN', amount:0, createdAt: Date.now() ,remark:inparam.id,project:'购买产品'})
